...
plotSpatialFieldCL <- function(dir, name, minw, maxw, year_of_assess){
library(inlabru)
library(sf)
library(ggpubr)
...
p[[length(years)+1]]<- ggplot(Cor.data,aes(x=d.vec/1000, y=Cor.M))+
  geom_line()+basetheme +xlab("Distance (km)") + ylab("Correlation")+ geom_vline(xintercept = Range/1000, lty=2)+ggtitle("")#+
 # scale_x_continuous(breaks=seq(0, Range*3/1000, 10), limits=c(0, Range*3/1000))



ggarrange(plotlist = p, ncol=7, nrow=second_panel, legend="top", common.legend = T)
...
if(grepl(name, pattern="ZAG")){
  ...
  second_panel<-ceiling((length(years)+1)/7)
  #set.panel(2,second_panel) # 2X2 matrix of plots
  g <-  list()
  b<-  list()
  ...
  ggarrange(plotlist = b, ncol=7, nrow=second_panel, legend="top", common.legend = T)
  ggsave(paste0("figures/",year_of_assess,"/INLA/random_field/",name,"_ZAG_bernouilli.png"), dpi=600, units="in", width=2*7, height=2*second_panel)
  
  
  ggarrange(plotlist = g, ncol=7, nrow= second_panel, legend="top", common.legend = T)
  ggsave(paste0("figures/",year_of_assess,"/INLA/random_field/",name,"_ZAG_gamma.png"), dpi=600, units="in",width=2*7, height=2*second_panel)
