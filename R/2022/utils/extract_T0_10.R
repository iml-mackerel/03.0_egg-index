...
egg <- readRDS(paste0("data/",year,"/PL_Bongo_Scomber_eggs_larvae_Counts_L2_", year, ".RDS"))
dat_out <- data.frame()

#no data in 95,97 and 2020

#read and get uniform format
for(year in c(1987:1994,1996, 1998:2019, 2021:year)){
  if(year <= 2000){
  ...
  if(year >=2024){
    ...
    daty <-  read.delim(filey, header = FALSE, sep=",",skip = 1, col.names = c("file","station","datetime","lat","lon","prof_max","temperature0_10","salinity","sigma_t")) %>%       
      mutate(station=as.character(station), year=year, date=substring(datetime, 1,10)) %>%  dplyr::select(-datetime)
  }
