...
INLA_ZAG <- function(dat, res_mesh, res_pred, Rvar="DEP", year, trajet){
...
#default to log link note link=1
Ggamma <- inla(fGampos,
               family = "gamma", 
               #lincomb = All.lcs,
...
exi<- exists(paste0("results/",year,"/INLA/",Rvar,"_INLA_mesh", res_mesh,"km_pred_",res_pred,"_ZAG",year,"_trajet",trajet,".RData"))
overw =NA
if(exi) overw<- menu(c("Yes", "No"), title = "Model already exist. Do you want to overwrite?")
if(!exi | overw ==1) save(Gbern, Ggamma, dat,df.grid, mesh,wReplBern.index ,wReplGamma.index,stkallBern, stkallGamma, spde,file=paste0("results/",year,"/INLA/",Rvar,"_INLA_mesh", res_mesh,"km_pred_",res_pred,"_ZAG",year,"_trajet",trajet,".RData"))
