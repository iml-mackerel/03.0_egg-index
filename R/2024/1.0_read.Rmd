...
if(new){
extract_biochem(year=my.year, path_to_file = NULL)
extract_T0_10(year=my.year)
}
```

```{r filter, message = F}
lookup <- read.delim("data/lookup_station_egg.txt") # position and depth of stations.

bat <- getNOAA.bathy(-70,-58,40,52, res = 4, keep = TRUE)
...
ggsave(paste0("img/",my.year,"/area_of_interest.png"), width = 6, height = 6, units = "in", dpi = 600)
...
write.table(tableN, paste0("results/",my.year,"/tableN_trajets.txt"), sep = "\t", dec = ".", row.names = F)
...
if(!file.exists(paste0("data/",my.year,"/INLA/INLA_CV_eggdata.RData"))){ #prevent overwriting
save(eggcv_to_fit, eggcv_all, file=paste0("data/",my.year,"/INLA/INLA_CV_eggdata.RData"))
}

save(eggt1,  file=paste0("data/",my.year,"/eggt1.RData"))
...
ggsave(paste0("img/",my.year,"/Incubation_timeBI.png"), width=6, height=4, dpi=600, units="in")  
```
...
ggsave(paste0("img/", my.year,"/MAP_egg_cover.png"), width=8, height=5, dpi=600, units="in")
...
ggsave(paste0("img/", my.year,"/MAP_egg_temperature_flip.png"), width=6, height=7, dpi=600, units="in", bg="transparent")
...
ggsave(paste0("img/", my.year,"/Volume_validation.png"), width=6, height=4, dpi=600, units="in")
...
eggt1filter<-eggt1filter%>%  dplyr::mutate(outliers_depth = if_else(DEP==0, "No egg", outliers_depth),
                                     ...
                                     outliers_depthFR= recode_factor(outliers_depth, `Sans oeuf`="Sans oeuf",`Depth within CI` = "Profondeur à l'intérieur des IC", 
                                                                     ...
                                                                     `Depth > CI` = "Profondeur > IC")) %>%  as_tibble() %>% dplyr::filter(year %in% c(1996,(as.numeric(as.character(my.year))-1): as.numeric(as.character(my.year)))) 
...
ggsave(paste0("img/", my.year,"/Depth_validation.png"), width=7, height=4, dpi=600, units="in")
