...
xy <- create_coord(dat[, c("longitude", "latitude")],crsin=projdeg, crsout=lcc)
dat$X.m <- xy$x
dat$Y.m <- xy$y
Loc <- cbind(dat$X.m, dat$Y.m)

mesh <- readRDS(paste0("data/",year_of_assess,"/INLA/mesh",res_mesh,"km_",year_to_report,"_tajet",trajet,".rds"))
