...
new=T
root<- "../../"
year_to_plot= my.year # can be changed to (my.year-1) : my.year when 2 years are reported
...
extract_biochem(my.year=my.year, path_to_file = NULL) # if warning about maqytr then probably wrong locale Rstart R

#"S:/Pélagiques/Plancton/Relevés/Relevé 2025/Métadonnées_IML2025-22.xlsx"
}
```

```{r biochemsp2, message = F}
if(new){
 setwd(rprojroot::find_rstudio_root_file())
  extract_T0_10(my.year=my.year)
...
eggt1 <- egg %>% mutate(mission_name=coalesce(mission_name, as.character(year))) %>%  dplyr::filter(!year %in% c(1982), trajet==1, !stringi::stri_detect_regex(mission_name, pattern="IML-2009-47"))
...
```{r incub}
egg_temp = eggt1 %>%  filter(year %in% year_to_plot) %>%  dplyr::select(year, temperature0_10) %>% mutate(temperature0_10 = round(temperature0_10)) %>%  group_by(year, temperature0_10) %>%  tally() %>%  rename(Incubation=n)
...
```{r covarage}
eggmap <-  eggt1 %>%  filter(year %in% year_to_plot) %>%  mutate(date=ymd(date))
look2<- expand_grid(lookup, year=year_to_plot) %>%  filter(!paste(station, year) %in% paste(eggmap$station, eggmap$year))
...
basemap2 + geom_path(data=emap, aes(x=longitude, y=latitude)) + geom_sf(data=egg2 %>%  filter(!is.na(doy)), aes(fill= temperature0_10), size=3, shape=21) + 
  ...
  theme(legend.position = "bottom", legend.key.width = unit(1, "cm"), plot.background = element_blank())
...
egg_d <- egg_d %>% dplyr::mutate(
  PA = ifelse(DEP == 0, 0, 1),
  colmatage = if_else(str_detect(ge_collector_comment, pattern = "colm"), "Y", "N")
)
