...
my.year <- 2025 # maximum year to extract data
...
df <- bio %>%
  ...
  arrange(year)
#removing gsi > 40 is jutified because it does not concur with stages. Stage 1 and 8 at > 40% is unlikely. Maybe a 0 missing in weight. 

df <- df %>%
  arrange(year) %>%
  mutate(year = as.numeric(as.character(year))) %>%  filter(!is.na(gsi), !is.na(doy), !is.na(year), !sex=="I") %>% #there is only 2 I 
  mutate(maturcat = ifelse(!is.na(matur) & matur == 6, "6",                                                                                                                                 ifelse(is.na(matur), "other", "other"))) %>%  ungroup()
...
df %>% filter(year %in% year_to_plot) %>% ggplot(aes(x=doy, y=gsi, shape=sex)) +geom_point(aes(col=maturcat)) + facet_grid(sex~year) + scale_color_manual(values = c("green3", "grey")) +scale_shape_manual(values=c(5,17))
...
write.table(table_prov, paste0(root,"results/",my.year,"/spawning/sensitivity/table_provenance_ech.txt"), row.names=F, sep="\t", dec=".")
...
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/NsamplesBI.png"), width=6, height=5, dpi=600, units="in")
...
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/Nsamples_stagesBI.png"), width=6, height=5, dpi=600, units="in")
...
write.table(propdoy, paste0(root,"results/",my.year,"/spawning/sensitivity/proportion_stages_doy.txt"), row.names=F, sep="\t", dec=".")
...
df %>%  ggplot(aes(x=year, fill=sex))+geom_bar()
df %>%  ggplot(aes(x=year, fill=sex))+geom_bar(position="fill")
...
###model with sex as factor does not converge
...
save(nlme.fixAR,start.val,df, nlme_coefAR, file = paste0(root,"results/", my.year,"/spawning/sensitivity/nlmeAR", my.year,".RData"))
} else{load(paste0(root,"results/", my.year,"/spawning/sensitivity/nlmeAR", my.year,".RData"))}
...
save(nlme.fixpredsAR1, file = paste0(root,"results/",my.year,"/spawning/sensitivity/predicted_propspawn_doy", my.year,".RData"))
} else{load(file = paste0(root,"results/",my.year,"/spawning/sensitivity/predicted_propspawn_doy", my.year,".RData"))
}
...
write.table(tabl_out, paste0(root,"results/",my.year,"/spawning/sensitivity/table_nlmeAR_", my.year, ".txt"), row.names = F, sep = "\t", dec = ".")
...
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/nlme_spawning_proportion_", my.year,"EN3.png"), width = 6, height = 6, dpi = 600, units = "in")
spEN3
...
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/nlme_spawning_proportion_", my.year, "BI3.png"), width = 6, height = 6, dpi = 600, units = "in")
```
...
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/spawning_season_", my.year, "EN.png"), width = 7, height = 5, dpi = 600, units = "in")

pt2FR <-  pt2+ylab("Jour de l'ann\u00E9e") + xlab("Ann\u00E9e") 
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/spawning_season_", my.year, "FR.png"), width = 7, height = 5, dpi = 600, units = "in")

pt2BI <-  pt2+ylab("Jour de l'ann\u00E9e / Day of year") + xlab("Ann\u00E9e / Year")
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/spawning_season_", my.year,"BI.png"), width = 7, height = 5, dpi = 600, units = "in")
...
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/spawning_season_", my.year, "EN2.png"), width = 7, height = 5, dpi = 600, units = "in")
...
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/spawning_season_", my.year, "BI2.png"), width = 7, height = 5, dpi = 600, units = "in")
...
ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/spawning_season_", my.year, "FR2.png"), width = 7, height = 5, dpi = 600, units = "in")
...
```{r spawn bias1, message = F}
  PESd<- read.delim(paste0(root,"results/",my.year,"/spawning/sensitivity//table_nlmeAR_",my.year,".txt"))  %>% dplyr::select(year, xmid)
  
  propstages<- read.delim(paste0(root,"results/",my.year,"/spawning/sensitivity/proportion_stages_doy.txt"))
...
sd_diff<- wa %>%  filter(year!=2022)%>%  reframe(meandiff_xmid6= sd(xmid-maxstage6, na.rm=T) )
 bias<-  ggplot(data=wa %>%  filter(year !=2022),aes(x=xmid, y=maxstage6)) +geom_abline(intercept=0,slope=1, col="red", lty=2)+geom_abline(intercept=-1.96*sd_diff$meandiff_xmid6,slope=1, col="black", lty=2)+geom_abline(intercept=1.96*sd_diff$meandiff_xmid6,slope=1, col="black", lty=2)+geom_label(aes(label=year, fill=for_fill), alpha=0.5)+
 ...
 ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/bias_indicator_spawning_EN.png"), width=6, height=5, dpi=600, units="in")
 biasEN
 
 biasFR <- bias + ylab("Jour de l'année de la proportion maximale de stade 6") + xlab("Jour de l'année du pic de ponte (xmid)")
 ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/bias_indicator_spawning_FR.png"), width=6, height=5, dpi=600, units="in")
 ...
 ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/bias_indicator_spawning_BI.png"), width=6, height=5, dpi=600, units="in")
 
``` 

##### Proportion of larvae
```{r spawn larvae, message = F}
  load(paste0(root,"results/",my.year,"/spawning/sensitivity/predicted_propspawn_doy", my.year, ".RData"))
  ...
  ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/Valdiation_spawning_larvesEN2.png"), width=6, height=5)

  
  ggarrange(pl1EN, pl2EN, common.legend=T, legend="bottom", ncol=1)
  ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/Valdiation_spawning_larvesEN.png"), width=9, height=8)
  ...
  ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/Valdiation_spawning_larvesFR.png"), width=9, height=8)
  ...
  ggBI<- ggarrange(pl1BI, pl2BI, common.legend = T, legend="bottom", ncol=1)
   ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/Valdiation_spawning_larvesBI.png"), width=9, height=8)
  pl2BI
  ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/Valdiation_spawning_larvesBI2.png"), width=6, height=5)

  
  
   dur<- read.delim(paste0(root,"results/",my.year,"/spawning/sensitivity/table_nlmeAR_", my.year, ".txt")) %>%  dplyr::filter(!year %in% c(1980:1982,1991, 1999,2020))
  ...
  ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/durationEN.png"), width=7, height=4, dpi=600, units="in")
  pduFR<- pdu +scale_x_continuous(breaks=seq(1980, my.year, 5), name="Ann\u00E9e")+ ylab("Durée de la ponte (jours)") 
  ggsave(paste0(root,"img/",my.year,"/spawning/sensitivity/durationFR.png"), width=7, height=4, dpi=600, units="in")
