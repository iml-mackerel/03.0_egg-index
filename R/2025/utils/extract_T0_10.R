...
library(sf)
library(nngeo)
extract_T0_10 <-  function(year_to_report, year_of_assess){
...
for(year in c(1987:1994,1996, 1998:2019, 2021:year_to_report)){
  if(year <= 2000){
...
egg_joined79_86<- full_join(egg_joined %>%  filter(year < 1987) %>%  dplyr::select(-temperature0_10),t79_86) 
...
write.table(egg_joined, paste0("data/",year_of_assess,"/PL_Bongo_Scomber_eggs_larvae_Counts_L2_", year_to_report, ".tsv"), sep="\t", dec=".", row.names=F)
}
