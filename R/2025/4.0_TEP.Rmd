...
```{r read spawn.sens, message = F}
    # pour date med
spaw.med <- read.delim(paste0(pwd, "results/",my.year,"/spawning/sensitivity/table_nlmeAR_", my.year, ".txt"))  %>% 
  ungroup()
...
write.table(TEP_output, file=paste0(pwd, "csv/",my.year,"/TEP_", my.year,"sensitivity.csv"), row.names=F, sep=",", dec=".")
```




```{r read spawn.plot, message = F}
TEP_output1<- read.csv(paste0(pwd, "csv/",my.year,"/TEP_", my.year,".csv")) %>%  
  dplyr::select(year, TEP) %>% mutate(run="base") 
TEP_output2<- read.csv(paste0(pwd, "csv/",my.year,"/TEP_", my.year,"sensitivity.csv")) %>%  
    dplyr::select(year, TEP) %>% mutate(run="female")

TEP_sens<- bind_rows(TEP_output1, TEP_output2)



pt <- ggplot(data = TEP_output1, aes(x = year, y = TEP)) +
  geom_point() +
...
pt <- ggplot(data = TEP_sens, aes(x = year, y = TEP, col=run)) +
  geom_point() +
...
pt2<- TEP_sens %>%  
  pivot_wider(names_from=run, values_from=TEP) %>%  
  mutate(relerror= (female-base)/base) %>%
ggplot(aes(x = year, y = relerror)) +
  geom_bar(stat="identity") +
  theme_few() +
  theme(legend.position = c(0.8, 0.7), legend.title = element_blank())+
  geom_hline(yintercept=0, col="red", lty=2)

ptEN2 <- pt2 +   scale_x_continuous(name = "Year", breaks = seq(1980, max(spawn.out2$year), 5)) +
  ylab("Relative error")

ggarrange(ptEN1, ptEN2, nrow=2)

ggsave(paste0(pwd,"img/",my.year,"/Total_egg_production_sensitivity_EN.png"), width = 8, height = 5, unit = "in", dpi = 600)
