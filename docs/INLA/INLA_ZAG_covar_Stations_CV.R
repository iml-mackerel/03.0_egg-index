...
INLA_ZAG_CV <- function(dat, res_mesh, res_pred, Rvar="DEP", trajet, year){
...
overw=NA
exi<- exists(paste0("results/",year,"/INLA/DEP_INLA_mesh", res_mesh,"km_pred_",res_pred,"_ZAG_",year,"_CV_trajet",trajet,".RData"))
if(exi) overw<- menu(c("Yes", "No"), title = "Model already exist. Do you want to overwrite?")
if(!exi | overw ==1) save(Gbern, Ggamma, dat, mesh,wReplBern.index ,wReplGamma.index,stkallBern, stkallGamma, spde,file=paste0("results/",year,"/INLA/DEP_INLA_mesh", res_mesh,"km_pred_",res_pred,"_ZAG_",year,"_CV_trajet",trajet,".RData"))
