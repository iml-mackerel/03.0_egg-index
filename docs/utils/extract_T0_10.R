...
extract_T0_10 <-  function(my.year){
...
file_list <-  file_list[!grepl(file_list, pattern="Profils_moyen")]
...
for(year in c(1987:1994,1996, 1998:2019, 2021:my.year)){
  if(year <= 2000){
    filey<- file_list[grep(file_list, pattern=year)]
    daty <-  read.delim(filey, header = FALSE, sep=",",skip = 1, col.names = c("file","station","datetime","lat","lon","temperature0_10","salinity","sigma_t")) %>% 
      mutate(station=as.character(station), 
             date=substring(datetime, 1,11), year=year) %>% dplyr::select(-datetime)
  }
  if(year >2000 & year < 2021){
    ...
    daty <-  read.delim(filey, header = FALSE, sep=",",skip = 1, col.names = c("file","station","date","time","lat","lon","temperature0_10","salinity","sigma_t")) %>%       
      mutate(station=as.character(station), year=year) %>%  dplyr::select(-time)
  }
  if(year == 2021 | year ==2022){
    filey<- file_list[grep(file_list, pattern=year)]
    daty <-  read.delim(filey, header = FALSE, sep=",",skip = 1, col.names = c("file","station","temperature0_10","salinity","sigma_t")) %>%  
      mutate(station=as.character(station), date=NA, lat=NA, lon=NA, year=year)
  }
  if(year ==2023){
    filey<- file_list[grep(file_list, pattern=year)]
    daty <-  read_excel(filey,sheet= "Moyenne_Bionet_10m_surface",skip=1,col_names = c("file","station","datetime","lat","lon","prof_max","temperature0_10","salinity","sigma_t")) %>%  
      mutate(station=as.character(station), date=NA, lat=NA, lon=NA, year=year, date=format(ymd(substring(datetime, 1,10)),"%d-%m-%Y")) %>% 
  ...
  if(year >=2024){
    ...
    daty <-  read.delim(filey, header = FALSE, sep=",",skip = 1, col.names = c("file","station","datetime","lat","lon","prof_max","temperature0_10","salinity","sigma_t")) %>%       
      mutate(station=as.character(station), year=year, date=format(ymd(substring(datetime, 1,10)),"%d-%m-%Y")) %>%  dplyr::select(-datetime)
  }
...
dat_out <- dat_out %>% mutate(#year=as.numeric(str_sub(date, 8,11)),
  ...
  month=gsub(month, pattern="-", replacement=""),
  month = as.numeric(ifelse(month %in% c("Jun", "JUN"),as.numeric(6),
                            ifelse(month %in% c("JUL", "Jul"),as.numeric(7), month))),
  day=str_sub(date,1,2),
  date = lubridate::ymd(ifelse(!is.na(date),as.character(paste(year,month,day,sep = "-")), NA)))
...
egg_joined79_86<- full_join(egg_joined %>%  filter(year < 1987) %>%  dplyr::select(-temperature0_10),
                            t79_86 %>%  mutate(station=as.character(station),
...
mutate(
  temperature0_10 = coalesce(temperature0_10, ctemp)) %>%  dplyr::select(-ctemp)
  

write.table(egg_joined, paste0("data/",my.year,"/PL_Bongo_Scomber_eggs_larvae_Counts_L2_", my.year, ".tsv"), sep="\t", dec=".", row.names=F)
write_rds(egg_joined, paste0("data/",my.year,"/PL_Bongo_Scomber_eggs_larvae_Counts_L2_", my.year, ".RDS"))
